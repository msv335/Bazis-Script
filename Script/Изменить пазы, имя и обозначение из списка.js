...
Необходимо улучшить скрипт для выбора наименований пазов из списка
*/
...
const CUT = ["Выемка", "Выемка торцевая", "Гравировка", "Еврокромка", "Карандаш",
    "Паз", "Паз фигурный", "Паз технологический", "Паз торцевой",
    "Скос по торцу", "Фацет", "Фаска"]
...
for (let key in listCut) {
    ...
    for (c = 0; c < CUT.length; c ++ ) {
        let cut = CUT[c]
        Menu = Window[nameNum].DropDownMenu.NewButton(CUT[c])
        Menu.OnClick = function() {
            Window[nameNum].Value = cut
        }
    }
...
//-- window form events
paramsCut.forEach(function (elem) { // обработка изменений в наименованиях
